...
let logkanal = await db.fetch(`log_${message.guild.id}`)
  ...
  if (args[0] === "sıfırla" || args[0] === "kapat") {
    if(!logkanal) return message.channel.send(`<a:hypesquad1:761965318356336710> **Modlog Kanalı Zaten ayarlı değil**`);
    db.delete(`log_${message.guild.id}`)
   message.channel.send(`<a:hypesquad1:761965318356336710> **ModLog Kanalı başarıyla sıfırlandı.**`);
  
    return
  }
  
if (!logk) return message.channel.send(`<a:hypesquad1:761965318356336710> **Bir modlog kanalı belirtmelisin.**`);
